...
export const getNounSeedFromBlockHash = (nounId: BigNumberish, blockHash: string): NounSeed => {
  ...
  return {
    background: getPseudorandomPart(pseudorandomness, bgcolors.length - 2, 0),
    body: getPseudorandomPart(pseudorandomness, bodies.length - 2, 48),
    accessory: getPseudorandomPart(pseudorandomness, accessories.length - 2, 96),
    head: getPseudorandomPart(pseudorandomness, heads.length - 2, 144),
    glasses: getPseudorandomPart(pseudorandomness, glasses.length - 2, 192),
  };
};
